...
function visitor(
  ...
  const visitor: ts.Visitor = (node: ts.Node): ts.Node => {
    if (
      ...
      const importPathWithQuotes = (node as ts.ImportDeclaration).moduleSpecifier.getText(
        sf
      );
      const importPath = importPathWithQuotes.substr(
        1,
...
function amdVisitor(
  ...
  const visitor: ts.Visitor = (node: ts.Node): ts.Node => {
    if (isDefineNode(node)) {
      const importPathsWithQuotes = ((node as ts.CallExpression)
        .arguments[0] as ts.ArrayLiteralExpression).elements;
